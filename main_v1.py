...
from page.geoSetPage import GeometrySettingPage
...
class MainWindow(QMainWindow):
    ...
    def switch_to_3d_view(self):
        ...
        self.graphics_view = ThreeDGraphicsView()
        self.graphics_layout.addWidget(self.graphics_view)

    def clear_graphics_container(self):
    ...
    def open_tree_menu(self, position):
        ...
        if self.is_2d is None:
                ...
                if action == copy_action:
                    ...
                    if ok and new_name:
                        if new_name in self.geometry_pages:
                            ...
                            new_page = GeometrySettingPage(self.graphics_scene, old_page.shape_type, new_name)
            ...
            if item.text(0) == "几何":
                ...
                if action in (new_cube_action, new_sphere_action):
                    shape_type = "长方体" if action == new_cube_action else "球体"
                    shape_name, ok = QInputDialog.getText(self, "命名图形", f"请输入{shape_type}名称：")
                    if ok and shape_name:
                        self.add_new_shape(shape_type, shape_name)
                ...
                if action == copy_action:
                    ...
                    if ok and new_name:
                        if new_name in self.geometry_pages:
                            ...
                            old_page = self.geometry_pages[shape_name]
                            new_page = GeometrySettingPage(self.graphics_view, old_page.shape_type, new_name, is_2d=False)
    ...
    def add_new_shape(self, shape_type, shape_name):
        ...
        if self.is_2d:
            page = GeometrySettingPage(self.graphics_scene, shape_type, shape_name, is_2d=self.is_2d)
        else:
            page = GeometrySettingPage(self.graphics_view, shape_type, shape_name, is_2d=self.is_2d)
